...
function displayCards(difficulty) {
  for (let i = 1; i < gamePattern.length + 1; i++) {
    $("#reference").after("<div class='card-div card-div-size1'><input class='card " + i + "' onclick='buttoclicked(" + i + ")' type='image' src='Imgs/card.png'></div>");
  }
}
...
function resizeGameDiv(difficulty) {
  switch (difficulty) {
    ...
    case 18:
      ...
      // $(".card-div").removeClass("card-div-size1 card-div-size2 card-div-size3 card-div-size4");
...
function restartGame(difficulty) {
  ...
  setTimeout(function() {
    ...
    gamePattern = [];
    $(".card-div").remove();
    generateGameCards(difficulty); //Regenerates the game cards
    if ($(".card").hasClass("flip")) { // removes flip states from cards
...
function changeDifficulty(newDif) {
  ...
  $(".difficulty-selector-button").removeClass("btn-light btn-outline-success btn-outline-warning btn-outline-danger btn-outline-dark");// removes all previous classes
switch (difficulty){             // applies the current difficulty to the selector
  case 4:
    $(".difficulty-selector-button").toggleClass("btn-outline-success");
    $(".difficulty-selector-button").text("Fácil");
    break;

  case 9:
    $(".difficulty-selector-button").toggleClass("btn-outline-warning");
    $(".difficulty-selector-button").text("Médio");
    break;

  case 18:
    $(".difficulty-selector-button").toggleClass("btn-outline-danger");
    $(".difficulty-selector-button").text("Difícil");
    break;

  case 35:
    $(".difficulty-selector-button").toggleClass("btn-outline-dark");
    $(".difficulty-selector-button").text("Extremo");
    break;
}
